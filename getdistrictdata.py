...
if (prevdate != currentdate):
    for date in [date1, date2, 'district' + date2]:
        ...
        try:
            ...
            over10percent = df[['State', 'District', 'Positivity']].dropna (thresh=1).iloc[:-1, :].fillna (
                method='ffill')
            over10percent['Positivity'] = over10percent['Positivity'] / 100

            between5and10percent = df[['State.1', 'District.1', 'Positivity.1']].dropna (thresh=1).iloc[:-1, :].fillna (
                method='ffill').rename (
                columns={"State.1": "State", "District.1": "District", "Positivity.1": "Positivity"})
            between5and10percent['Positivity'] = between5and10percent['Positivity'] / 100

            under5percent = df[['State.2', 'District.2', 'Positivity.2']].dropna (thresh=1).iloc[:-1, :].fillna (
                method='ffill').rename (
                columns={"State.2": "State", "District.2": "District", "Positivity.2": "Positivity"})
            under5percent['Positivity'] = under5percent['Positivity'] / 100

            output = pd.concat ([over10percent, between5and10percent, under5percent]).sort_values (
                by=['State', 'District']).rename (columns={'Positivity': 'Test Positivity Rate'})
            output['Date'] = currentdate
            output.to_csv ("districtdata.csv", columns=['Date', 'State', 'District', 'Test Positivity Rate'],
                           header=False, index=False, mode='a', float_format="%.4f")
