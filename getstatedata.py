...
states = ['Andaman and Nicobar Islands', 'Andhra Pradesh', 'Arunachal Pradesh', 'Assam', 'Bihar', 'Chandigarh',
          ...
          'Himachal Pradesh', 'India', 'Jammu and Kashmir', 'Jharkhand', 'Karnataka', 'Kerala', 'Ladakh', 'Lakshadweep',
...
try:
    ...
    for state in states:
        ...
        for dt in daterange (date (2021, 4, 1), today.date ()):
            ...
            try:
                stateweeklycases = statecases[statecases.index == t0].iloc[0] - statecases[statecases.index == t1].iloc[
                    0]
                stateweeklytests = statetests[statetests.index == t0].iloc[0] - statetests[statetests.index == t1].iloc[
                    0]
    ...
    df.to_csv ("statedata.csv", columns=['Date', 'State', 'Weekly Cases', 'Weekly Tests', 'Test Positivity Rate'],
